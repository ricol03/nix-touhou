  ...
  outputs = { self, nixpkgs, flake-utils, rust-overlay, gitignore }:
    ...
    in {
      packages.x86_64-linux = rec {
        ...
        touhouTools = rec {
          #makeWinePrefix =
          #  { defaultFont ? "Noto Sans CJK SC", fontPackage ? pkgs.noto-fonts-cjk-sans }:
          #  (pkgs.callPackage ({ stdenvNoCC, wine, pkgsCross, bash }:
          #    stdenvNoCC.mkDerivation {
          #      name = "touhou-wineprefix";
          #      nativeBuildInputs = [ wine ];
          #      phases = [ "installPhase" ];
          ...
          #        wineserver --wait || true
          #        echo Setting up dxvk.
          #        dxvk32_dir=${pkgsCross.mingw32.dxvk_2}/bin mcfgthreads32_dir=${pkgsCross.mingw32.windows.mcfgthreads_pre_gcc_13}/bin ${bash}/bin/bash ${
          #          ./setup_dxvk.sh
          #        }
          #        echo dxvk installed.
          #        wineserver --wait || true
          ...
          #    }) { });
          defaultWinePrefix = makeWinePrefix { };
          #makeTouhou = { thVersion, name ? thVersion, enableVpatch ? true, enableThprac ? true
          #  , thcrapPatches ? null, thcrapSha256 ? "", baseDrv ? null
          #  , winePrefix ? defaultWinePrefix }:
          #
          #  pkgs.callPackage
          #  ({ stdenvNoCC, lib, bash, makeWrapper, writeScript, wine, bubblewrap, iconv, dxvk }:
          #    assert (builtins.hasAttr thVersion touhouMetadata);
          #    let
          #      pkgname = "${name}-wrapper";
          #      metadata = touhouMetadata."${thVersion}";
          #      setAppdata = appdata:
          #        writeScript "setappdata-and-run" ''
          #          @echo off
          ...
          #      thcrapConfigPath = if thcrapPatches != null then
          #        thcrapDown {
          #          sha256 = thcrapSha256;
          #          patches = thcrapPatches;
